 ...
 *
 * Ovaj fajl:
 * - uÄitava projekte i statistiku iz api.php
...
/**
 * ZaÅ¡tita teksta prije prikazivanja unutar HTML-a.
 */
...
/**
 * Naziv naÄina donacije.
 */
function paymentName(payment) {
    const names = {
        gotovina: "Gotovina",
        racun: "Bankovni raÄun",
 ...
 * prijave kao klik na <dialog>, zbog Äega bi se forma zatvorila.
 *
 * Modal se zatvara samo:
 * - dugmetom za zatvaranje
 * - dugmetom Odustani
 * - tipkom Escape, Å¡to <dialog> podrÅ¾ava automatski
 */
...
async function api(action, options = {}) {
    ...
    if (!response.ok) {
        throw new Error(data.error || "DoÅ¡lo je do greÅ¡ke.");
    }
    ...
    projectsContainer.innerHTML = `
        <div class="col-12 text-center py-5">
            <div class="spinner-border text-success" role="status">
                <span class="visually-hidden">UÄitavanje...</span>
            </div>
        </div>
    `;
...
function renderStatistics(totals) {
    ...
    document.getElementById("stats").innerHTML = `
        ...
        <div class="col-md-4">
            <div class="summary-card">
                <div>
                    <span>RASPOLOÅ½IVO STANJE</span>
                    <strong>${formatMoney(balance.ukupno_km, "BAM")}</strong>
                    <small class="d-block text-secondary mt-1">
                        Kurs: 1 â‚¬ = ${EUR_TO_BAM.toFixed(2)} KM
                    </small>
                </div>
    ...
    if (!donations.length) {
        return `
            <div class="empty-state py-5">
                ...
                <h3>Nema evidentiranih donacija</h3>
                <p class="mb-0">
                    Donaciju moÅ¾ete dodati u tabu â€žNovi unosâ€œ.
                </p>
            </div>
        `;
    }
                ...
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <span class="d-block text-secondary small">
                            ${escapeHtml(
                                donation.kontakt || "Kontakt nije naveden"
                            )}
                        </span>
                    </div>
                </div>
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div>
                ...
                <p class="small text-secondary mb-0">
                    Pregled svih evidentiranih donacija.
                </p>
            </div>
        </div>
...
function expenseTable(expenses, currency) {
    if (!expenses.length) {
        return `
            <div class="empty-state py-5">
                ...
                <p class="mb-0">
                    Rashod moÅ¾ete dodati u tabu â€žNovi unosâ€œ.
                </p>
            </div>
        `;
    }
    ...
    const rows = expenses.map(expense => `
        <tr>
            ...
            <td>
                ${escapeHtml(expense.broj_racuna || "â€”")}
            </td>
    ...
    return `
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div>
                ...
                <p class="small text-secondary mb-0">
                    Pregled utroÅ¡enih sredstava projekta.
                </p>
            </div>
        </div>
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-6">
                ...
                >
                    ...
                    <div class="mb-3">
                        <label class="form-label">NaÄin donacije</label>

                        <select name="nacin" class="form-select">
                            <option value="gotovina">Gotovina</option>
                            <option value="racun">Bankovni raÄun</option>
            ...
            <div class="col-lg-6">
                ...
                >
                    ...
                    <div class="mb-3">
                        <label class="form-label">DobavljaÄ</label>
                        <input
                            type="text"
                            name="dobavljac"
                            class="form-control"
                    ...
                    <div class="mb-3">
                        <label class="form-label">Broj raÄuna</label>
                        <input
                            type="text"
                            name="broj_racuna"
                            class="form-control"
...
/* =========================================================
   IZMJENA I BRISANJE POSTOJEÄ†IH ZAPISA
   ========================================================= */
        ...
        <form id="editRecordForm">
            <div class="dialog-header d-flex justify-content-between">
                <div>
                    ...
                    <p class="small text-secondary mb-0">
                        Izmijenite podatke i saÄuvajte promjene.
                    </p>
                </div>
                <button
                    type="button"
...
function editProject() {
    ...
    openEditor("Izmjena projekta", `
        ...
        <div class="row g-3 mt-0">
            <div class="col-md-6"><label class="form-label w-100">PoÄetak
                <input class="form-control" type="date" name="datum_pocetka"
                    value="${escapeHtml(project.datum_pocetka)}" required>
            </label></div>
            <div class="col-md-6"><label class="form-label w-100">ZavrÅ¡etak
        ...
        <div class="alert alert-warning small mt-3 mb-0">
            Promjena valute mijenja oznaku svih postojeÄ‡ih donacija i rashoda
            ovog projekta, bez preraÄunavanja iznosa.
        </div>
    `, async event => {
        ...
        try {
            ...
            showMessage("Projekat je uspjeÅ¡no izmijenjen.");
            await load();
...
async function deleteProject(id) {
    const confirmed = window.confirm(
        "Da li ste sigurni da Å¾elite obrisati ovaj projekat?\n\n" +
        "Bit Ä‡e trajno obrisane i sve njegove donacije i rashodi."
    );
...
function editDonation(id) {
    ...
    openEditor("Izmjena donacije", `
        ...
        <label class="form-label w-100 mt-3">NaÄin
    ...
    `, event => updateRecord(event, "donacije", id, "Donacija je izmijenjena."));
}


async function deleteDonation(id) {
    if (!window.confirm("Da li Å¾elite obrisati ovu donaciju?")) return;
    await deleteRecord("donacije", id, "Donacija je obrisana.");
}
...
function editExpense(id) {
    ...
    openEditor("Izmjena rashoda", `
        ...
        <label class="form-label w-100 mt-3">DobavljaÄ
            <input class="form-control" name="dobavljac"
                value="${escapeHtml(expense.dobavljac || "")}">
        </label>
        <label class="form-label w-100 mt-3">Broj raÄuna
...
async function deleteExpense(id) {
    if (!window.confirm("Da li Å¾elite obrisati ovaj rashod?")) return;
...
async function addEntry(event, type, projectId) {
    ...
    try {
        ...
        showMessage(
            type === "donacije"
                ? "Donacija je uspjeÅ¡no evidentirana."
                : "Rashod je uspjeÅ¡no evidentiran."
        );
...
projectForm.addEventListener("submit", async event => {
    ...
    try {
        ...
        showMessage("Projekat je uspjeÅ¡no saÄuvan.");
